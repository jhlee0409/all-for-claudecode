...
- **agents/** — Subagents: afc-architect, afc-security (persistent memory with `memory: project`), afc-impl-worker (ephemeral parallel worker with worktree isolation), 8 expert consultation agents (afc-backend-expert, afc-infra-expert, afc-pm-expert, afc-design-expert, afc-marketing-expert, afc-legal-expert, afc-appsec-expert, afc-tech-advisor — persistent memory, routed via `consult` command)
