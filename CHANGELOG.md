...
### Fixed
- **Command execution reliability**: Commands using `model: haiku` (doctor, init, checkpoint, resume, clean, principles) would ask "would you like to proceed?" instead of executing immediately. Root cause: haiku is too conservative with multi-step instructions. All commands now use `model: sonnet` for reliable execution.
- **AFC routing table wording**: Clarified "inform user on request" to "execute the skill immediately" to prevent models from describing commands instead of running them.
